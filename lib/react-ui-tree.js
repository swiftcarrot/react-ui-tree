...
module.exports = React.createClass({
  ...
  drag(e) {
    ...
    if(diffX < 0) { // left
      if(!index.next) {
        newId = tree.move(index.id, index.parent, 'after');
      }
    } else if(diffX > paddingLeft) { // right
      if(index.prev && !tree.getIndex(index.prev).collapsed) {
        newId = tree.move(index.id, index.prev, 'append');
      }
    }

    if(diffY < 0) { // up
      var above = tree.getNodeByTop(index.top-1);
      newId = tree.move(index.id, above.id, 'before');
    } else if(diffY > dragging.h) { // down
      if(index.next) {
        var below = tree.getIndex(index.next);
        if(below.children && below.children.length && !below.collapsed) {
          newId = tree.move(index.id, index.next, 'prepend');
        } else {
          newId = tree.move(index.id, index.next, 'after');
        }
      } else {
        var below = tree.getNodeByTop(index.top+1);
        if(below && below.parent !== index.id) {
          if(below.children && below.children.length) {
            newId = tree.move(index.id, below.id, 'prepend');
          } else {
            newId = tree.move(index.id, below.id, 'after');
          }
        }
      }
    }

    if(newId) dragging.id = newId;
